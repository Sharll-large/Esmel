...
using std::vector, std::string, std::pmr::unordered_map, std::map, std::stack, std::nullptr_t, std::shared_ptr, std::unordered_set;
...
class EsmelInterpreter
{
    ...
    {
        // 已定义的变量
        if (stack_frame.top().variables.contains(origin)) return stack_frame.top().variables[origin];
        // 字符串
    ...
    // 调用一个非内置的esmel函数。
    {
        ...
        for (int i=0; i<func.arguments.size(); i++)
        {
            if (exec_stack.empty())
            {
                std::cerr << "Error: "<< func_name << " needs " << func.arguments.size() << " argument(s), but only " << i << " are given." << std::endl;
                error();
            }
    ...
    // 执行一段esmel代码。
    {
        for (int i=code.size()-1; i>=0; i--)
        {
            ...
            if (token == "goto")
            {
                string target = getString(exec_stack.top()); exec_stack.pop();
            ...
            {
                auto val = getBool(exec_stack.top()); exec_stack.pop();
            ...
            {
                auto o1 = exec_stack.top();exec_stack.pop();
                auto o2 = exec_stack.top(); exec_stack.pop();
            ...
            {
                EsmelObject target = exec_stack.top(); exec_stack.pop();
                exec_stack.push(createString(type_of(target)));
            }
            else if (token == "set")
            {
                EsmelObject var_name = exec_stack.top(); exec_stack.pop();
                EsmelObject val = exec_stack.top(); exec_stack.pop();
            ...
            {
                EsmelObject o1 = exec_stack.top(); exec_stack.pop();
                exec_stack.push(createBool(!bool_store_[o1.index]));
            }

            // 基本IO
            else if (token == "print") { std::cout << to_string(exec_stack.top());exec_stack.pop(); }
            else if (token == "println") { std::cout << to_string(exec_stack.top()) << std::endl;exec_stack.pop(); }

            // 数学
            else if (unordered_set<string>{"add", "sub", "mul", "div"}.contains(token))
            {
                if (token == "add")
                {
                    auto a1 = exec_stack.top(); exec_stack.pop();
                    auto a2 = exec_stack.top(); exec_stack.pop();
            ...
            // 其他自定义Esmel函数
            else if (esmel_functions.contains(token))
            {
                call(token);
            }
